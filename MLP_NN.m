...
InputValues = importedData.data(:, 1:length(importedData.data(1,:))-1);
OutputGuess = importedData.data(:, length(importedData.data(1,:)));
OutputGuess = OutputGuess - min(OutputGuess);
OutputActual = -1*ones(size(OutputGuess));

%% Calculate Number of Input and Output NodesActivations
nbrOfInputNodes = length(InputValues(1,:)); %=Dimention of Any Input InputValues
% nbrOfOutUnits = ceil(log2(length(unique(OutputGuess)))) + !; %Ceil(Log2( Number of Classes ))
...
InputValues = [ones(length(InputValues(:,1)),1) InputValues];

%% Calculate TargetOutputs %TODO needs to be general for any nbrOfOutUnits
TargetOutputs = zeros(length(OutputGuess), nbrOfOutUnits);
for i=1:length(OutputGuess)
    if (OutputGuess(i) == 1)
...
for Epoch = 1:nbrOfEpochs_max
    ...
    for Sample = 1:length(InputValues(:,1))
        %% Backpropagation Training
        %Forward Pass
        NodesActivations{1} = InputValues(Sample,:);
    ...
    MSE(Epoch) = sum((OutputActual-OutputGuess).^2)/(length(InputValues(:,1)));
    if (MSE(Epoch) == 0)
        zeroRMSReached = 1;
    end
    ...
    if (zeroRMSReached || mod(Epoch,draw_each_nbrOfEpochs)==0)
        ...
        unique_OutputGuess = unique(OutputGuess);
        training_colors = {'y.', 'b.'};
        ...
        xlim([min(InputValues(:,2))-margin max(InputValues(:,2))+margin]);
        ylim([min(InputValues(:,3))-margin max(InputValues(:,3))+margin]);
        for x = min(InputValues(:,2))-margin : step : max(InputValues(:,2))+margin
            for y = min(InputValues(:,3))-margin : step : max(InputValues(:,3))+margin
        ...
        for i = 1:length(unique_OutputGuess)
            points = InputValues(OutputGuess==unique_OutputGuess(i), 2:end);
